...
class Index extends NgRestModel
{
                            ...
                            $highlight = '<span style="background-color:#FFEBD1; color:black; margin: 0;">%s</span>')
    {
        ...
        if (!empty($module) && !empty($module->performance) && $module->performance == true) {
            $content = \Yii::$app->formatter->asHtml($this->content);
            //return $this->description;
            if (!empty($this->description)) {
                $content = \Yii::$app->formatter->asHtml($this->description);
            }
            $pos     = strpos(strtolower($content), strtolower($word));
            $lenWord = strlen($word);
            if ($pos !== false) {
                ...
                rsort($split);
                foreach ($split as $v) {
                    $pos     = strpos(strtolower($content), strtolower($v));
                    $lenWord = strlen($v);
                    if ($pos !== false) {
                        $newSplit = $split;
                        $searchK  = array_search($v, $split);
                        ...
                        return \Yii::$app->formatter->asHtml(html_entity_decode($ret));
                    }
                }
                //return $ret;
            }
     ...
     * @return string
     */
    public function getHighlightedTitle($words)
    {        
        $content = $this->title;
        $pos     = strpos(strtolower($content), strtolower($words));
        ...
        if ($pos !== false) {
            ...
            foreach ($split as $v) {
                ...
                if ($pos !== false) {
                    ...
                    $originalWord = substr($content, $pos, $lenWord);
                    $ret          = substr($content, 0, $pos).'<span style="background-color:#FFEBD1; color:black;">'.$originalWord.'</span>'.substr($content,
                            $pos + $lenWord);
                    foreach ($newSplit as $newV) {
                        $newPos = strpos(strtolower($ret), strtolower($newV));
                        if ($newPos !== false) {
        ...
        return \Yii::$app->formatter->asHtml(html_entity_decode($content));
    }
